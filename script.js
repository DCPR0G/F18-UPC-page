...
// HOVER EN BOTONES DEL WELCOME PARA ANIMAR EL F18 //
...
// AGREGAR CALCULADORA AL HTML //

const sectionCalculatorPage = document.querySelector(".hereCalculator");
...
const observer1 = new IntersectionObserver(verifyVisibilityCalc, {
    rootMargin: "-15%",
});

observer1.observe(grabButtonSec2)

const calculator = document.querySelector(".container-calc");

showAndHideLoad = () => {
    loading.style.transition = `opacity 500ms, transform 250ms`
    
    loading.style.opacity = 1
    loading.style.transform = `translate(0)`
    setTimeout(() => {
...
// OBSERVER PARA CARGAR TEXTO Y SEPARADOR SECCION 2 y
// HACER APARECER FLECHA DE AYUDA //

const divTextSec2 = document.querySelector(".div-text-up-sec2");
const separatorLineSec2 = document.querySelector(".separator-line");
const sectionCalculator = document.querySelector(".calculator");
const arrowHelp = document.querySelector(".fa-angle-left");


const verifyVisibilitySec2 = (entries) => {
    const entry = entries[0];
    if(entry.isIntersecting == true){
        arrowHelp.style.animationName = `move-arrow-help`
        divTextSec2.style.marginLeft = `0`;
        divTextSec2.style.opacity = 1;
        setTimeout(() => {
...
const observer2 = new IntersectionObserver(verifyVisibilitySec2,{
    rootMargin: "-30%"
});

observer2.observe(sectionCalculator);
